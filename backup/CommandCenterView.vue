    ...
    <section class="kpi-strip">
      <div v-for="m in metrics" :key="m.tag" class="kpi-tile" :class="'kpi-tile--' + m.tone">
        <div class="kpi-tile__badge">
          <SvgIcon :name="m.icon" :size="22" :color="iconColor(m.tone)" />
        </div>
...
/** tone → 徽章中 SVG 的描边色 */
function iconColor(tone: MetricTile['tone']): string {
  switch (tone) {
    case 'teal':
      return '#0d9488'
    case 'blue':
...
function buildMetrics(vo: StatsOverviewVO | null) {
  ...
  metrics.value = [
    { tag: '总收录量', num: formatNumber(safe.totalSamples), icon: 'package', tone: 'teal', delta: g },
    ...
    { tag: '今日新增', num: formatNumber(safe.todaySamples), icon: 'trend-up', tone: 'green', delta: 0 },
  ]
}
